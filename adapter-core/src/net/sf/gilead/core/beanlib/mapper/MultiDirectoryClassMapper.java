...
/**
 * Class mapper based on package hierarchy (Domain and DTO must have the same
 * name and placed in identified packages). This class mapper accepts multiple
 * distinct domain and dto packages.
 * 
...
public class MultiDirectoryClassMapper implements IClassMapper {
	...
	/**
	 * @param suffix
	 *            the suffix of all target classes (e.g. "DTO")
	 */
	...
	/**
	 * associate a source package (hibernated domain classes) with a 
	 * target package (gwt serializeable classes)
	...
	private String computeTargetClassName(Class<?> sourceClass) {
		...
		String strippedClassName = sourceClass.getCanonicalName().substring(
				sourcePackage.length());
		...
		if (_log.isInfoEnabled()) {
			_log.info("Source class name is " + sourceClass.getCanonicalName() +
					 ", target class is " + targetClassName);
		}
	...
	private static Logger _log = LoggerFactory
			.getLogger(MultiDirectoryClassMapper.class);
